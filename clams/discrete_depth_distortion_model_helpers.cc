...
void DiscreteDepthDistortionModel::visualize(const std::string &dir) const {
  ...
  int vert_divider = 20;
  cv::Mat3b overview(
      cv::Size(width_ * 2 + horiz_divider,
               height_ * num_layers + vert_divider * (num_layers + 2)),
      cv::Vec3b(0, 0, 0));
  std::vector<int> pub_layers;
  for (int i = 0; i < num_layers; ++i)
    pub_layers.push_back(i);
  // pub_layers.push_back(1);
  // pub_layers.push_back(2);
  // pub_layers.push_back(3);
  cv::Mat3b pub(cv::Size(width_, height_ * pub_layers.size() +
                                     vert_divider * (pub_layers.size() + 2)),
  ...
  for (int i = 0; i < num_layers; ++i) {
    ...
    for (int y = 0; y < mult.rows; ++y) {
      for (int x = 0; x < mult.cols; ++x) {
        ...
        if (val < 1)
          mult(y, x)[0] = std::min(255., 255 * (1.0 - val) / 0.25);
      }
    }
    ...
    for (int y = 0; y < count.rows; ++y) {
      for (int x = 0; x < count.cols; ++x) {
        const DiscreteFrustum &frustum =
            *frustums_[y / bin_height_][x / bin_width_];
        uchar val = std::min(255., (double)(255 * frustum.counts_(i) / 100));
        count(y, x)[0] = val;
        count(y, x)[1] = val;
        count(y, x)[2] = val;
      }
    ...
    cv::Mat3b combined(cv::Size(width_ * 2 + horiz_divider, height_),
                       cv::Vec3b(0, 0, 0));
    ...
    for (int y = 0; y < combined.rows; ++y)
      for (int x = 0; x < combined.cols; ++x)
        overview(y + i * (combined.rows + vert_divider) + vert_divider, x) =
            combined(y, x);
    ...
    for (int y = 0; y < pubmult.rows; ++y) {
      for (int x = 0; x < pubmult.cols; ++x) {
        ...
        if (val < 1) {
          pubmult(y, x)[1] = 255 - std::min(255., 255 * (1.0 - val) / 0.1);
          pubmult(y, x)[2] = 255 - std::min(255., 255 * (1.0 - val) / 0.1);
        }
      }
    }
    ...
    for (size_t j = 0; j < pub_layers.size(); ++j)
      if (pub_layers[j] == i)
        for (int y = 0; y < pubmult.rows; ++y)
          for (int x = 0; x < pubmult.cols; ++x)
            pub(y + j * (pubmult.rows + vert_divider) + vert_divider, x) =
                pubmult(y, x);
  }
