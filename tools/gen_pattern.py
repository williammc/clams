...
class PatternMaker:
  def __init__(self, cols,rows,output,units,square_size, radius_ratio,page_width,page_height):
    self.cols = cols
    self.rows = rows
    self.output = output
    self.units = units
  ...
  def makeCirclesPattern(self):
    ...
    r = spacing*self.radius_ratio# / 5.0 #radius is a 5th of the spacing TODO parameterize
  ...
  def makeACirclesPattern(self):
    spacing = self.square_size
    r = spacing*self.radius_ratio# / 5.0
...
def makePattern(cols,rows,output,p_type,units,square_size, radius_ratio,page_width,page_height):
    width = page_width
    spacing = square_size
    height = page_height
    r = spacing*radius_ratio# / 5.0
...
def main():
    ...
    try:
        opts, args = getopt.getopt(sys.argv[1:], "ho:c:r:T:u:s:w:h:g:", ["help","output","columns","rows",
    ...
    for o, a in opts:
        if o in ("-h", "--help"):
            ...
            square_size = float(a)
        elif o in ("-g", "--radius_ratio"):
            radius_ratio = float(a)
            if radius_ratio>=0.5 or radius_ratio<=0.0:
            	print "radius ratio must be in (0.0,0.5)"
            	sys.exit(0)
        elif o in ("-w", "--page_width"):
            page_width = float(a)
        elif o in ("-h", "--page_height"):
            page_height = float(a)
    pm = PatternMaker(columns,rows,output,units,square_size,radius_ratio,page_width,page_height)
