...
SlamCalibrationVisualizer::SlamCalibrationVisualizer(
  ...
  vis_.registerKeyboardCallback(&SlamCalibrationVisualizer::keyboardCallback,
                                *this);
  vis_.registerPointPickingCallback(
      &SlamCalibrationVisualizer::pointPickingCallback, *this);
...
void SlamCalibrationVisualizer::run() {
  // boost::thread thread_slam(boost::bind(&SlamVisualizer::slamThreadFunction,
  // this));
...
void SlamCalibrationVisualizer::setCamera(const std::string &camera_path) {
  const int argc = 3;
  char *argv[argc];
  argv[0] = (char *)std::string("aoeu").c_str();
  argv[1] = (char *)std::string("-cam").c_str();
...
void SlamCalibrationVisualizer::visualizationThreadFunction() {
  while (true) {
      ...
      *pcd = *map_;
      ...
      if (show_frame_) {
        ...
        if (traj.exists(frame_idx_)) {
          ...
          Eigen::Affine3f transform = traj.get(frame_idx_).cast<float>();
    ...
    vis_.addCoordinateSystem(
        0.3,
        calibrator_->trajectories_[seq_idx_].get(frame_idx_).cast<float>(), frame_idx_);
...
void SlamCalibrationVisualizer::pointPickingCallback(
...
void SlamCalibrationVisualizer::keyboardCallback(
